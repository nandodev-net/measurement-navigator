...
def request_fp_data(
    ...
    while next_url != None:
        try:
            ...
            if not page_req:
                f.write(
                    "Tiempo en request a ooni de una pagina de 200 mediciones parciales: "
                )
            ...
            if len(raw_object) > 0:
                ...
                try:
                    ...
                    start_time_datetime = datetime.datetime.strptime(
                        ms.measurement_start_time, "%Y-%m-%d %H:%M:%S"
                    )  # convert date into string
                    print(
                        c.green(
                            f"Trying to update cache, start time: {ms.measurement_start_time}, cache: {cache_min_date}. Is less: {start_time_datetime < cache_min_date}"
                        )
                    )
...
def update_measurement_table(
    n_measurements: int = None, test_name: str = None, retrys: int = -1
) -> dict:
    """
