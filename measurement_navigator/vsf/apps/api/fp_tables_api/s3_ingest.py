...
from apps.api.fp_tables_api.sync_measurements import (
    s3_measurements_download, s3_measurements_download_iter)
from apps.main.measurements.models import RawMeasurement
from apps.main.measurements.post_save_utils import (MeasurementCreator,
                                                    RawMeasurementBulker)
# Local imports
...
class S3IngestManager:
    ...
    def __init__(
        self,
        measurements_path: str = "./media/ooni_data/",
    ...
    def process_jsonl_file(
        self,
        file_name: str,
        cache_min_date: datetime.datetime,
        save_chunk_size: int = 10000,
        bulker: Optional[BulkCreateManager] = None,
    ):
        ...
        with path_to_jsonl.open("r") as file:
            for i, line in enumerate(file):
                ...
                test_start_time = datetime.datetime.strptime(
                    result["test_start_time"], "%Y-%m-%d %H:%M:%S"
                )
                measurement_start_time = datetime.datetime.strptime(
                    result["measurement_start_time"], "%Y-%m-%d %H:%M:%S"
                )
                ms = RawMeasurement(
                    input=measurement_input,
                    report_id=result["report_id"],
    ...
    def collect_incompatible_files(
        self, output_dir: str, incompatible_dir: str, cache_min_date: datetime.datetime
    ):
        ...
        for i, file_name in enumerate(file_list):
            ...
            if file_name.endswith(".jsonl"):
                ...
                try:
                    ...
                    print(
                        c.red(
                            f"[ERROR] Could not parse json file: {file_name}. Error: {e}"
                        )
                    )
    ...
    def _decompress_file(self, gz_file: str):
        ...
        try:
            ...
            print(
                c.yellow(
                    f"[WARNING] Could not find file {full_route} that was just decompressed"
                )
            )
    ...
    def process_raw_measurements(self, first_date: datetime.datetime):
        ...
        for i, raw_meas in enumerate(queryset_.iterator(chunk_size=1000)):
            ...
            measurement, subms = (
                self._measurement_creator.create_measurement_from_raw_measurement(
                    raw_meas
                )
            )
    ...
    def ingest(
        ...
        gz_list = (
            ...
            for file in s3_measurements_download_iter(
                test,
        ...
        for i, gzfile in enumerate(gz_list):
            ...
            try:
                ...
                print(
                    c.yellow(
                        f"Warning: trying to move file {json_file} from '{output_dir}' to '{incompatible_dir}', but couldn't find it"
                    )
                )
            except Exception as e:
                print(
                    c.red(
                        f"[ERROR] Could not finish processing json file content. Error: {e}"
                    )
                )
