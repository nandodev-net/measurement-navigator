...
from apps.main.sites.models import Domain, Site, SiteCategory
from apps.main.users import decorators
# Inheritance imports
from vsf.views import VSFLogin, VSFLoginRequiredMixin
...
class ListSites(VSFLoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        sites = Site.objects.values(
            "id", "name", "description_spa", "description_eng", "category__code"
        ).order_by("name")
    ...
    def post(self, request, *args, **kwargs):
        ...
        for element in assotiation:
            ...
            try:
                ...
                return JsonResponse({"error": {"type": "unvalid_site", "url": None}})
        ...
        for domain in db_domains:
            if domain.site != None:
                return JsonResponse(
                    {
                        "error": {
                            "type": "domain_already_added_to_a_site",
                            "domain": domain.domain_name,
                        }
                    }
                )
...
class SitesEndpoint(BaseDatatableView):
    ...
    def prepare_results(self, qs):
        json_data = []
        for item in qs:
            json_data.append({"associated": item.id, "name": item.name})
        return json_data
