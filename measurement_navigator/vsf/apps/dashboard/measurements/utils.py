...
class VSFCachedDatatableView(VSFLoginRequiredMixin, BaseDatatableView):
    ...
    def prepare_results_no_cache(self, qs: QuerySet) -> List[Dict[str, Any]]:
        raise NotImplementedError(
            "Implement prepare_results_no_cache in your submeasurement backend view in order to fill the datatables table"
        )

    def prepare_results(
        self, qs: QuerySet, cache_adjacent_pages: bool = False
    ) -> List[Dict[str, Any]]:
    ...
    @staticmethod
    def _generate_sql_querys_for_adjacent_pages(
        qs: QuerySet, num_adjacent_pages: int = 5
    ) -> List[str]:
        ...
        assert (
            "LIMIT" in original_sql_str
        ), "Can't generate previous pages without page size"
        ...
        for i in range(
            page_num - num_adjacent_pages, page_num + num_adjacent_pages + 1
        ):
