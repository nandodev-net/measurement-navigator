...
class ListSubMeasurementTemplate(VSFLoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        since = get.get("since")
        prefill["since"] = since or (datetime.now() - timedelta(days=10)).strftime(
            "%Y-%m-%d"
        )
...
class ListSubMeasurementBackend(VSFCachedDatatableView):
    ...
    def get_initial_queryset(self):
        ...
        qs = self.SubMeasurement.objects.select_related(
            "measurement", "measurement__raw_measurement"
        ).all()
...
class ListDNSBackEnd(ListSubMeasurementBackend):
    ...
    order_columns = ListSubMeasurementBackend.order_columns + ["dns_consistency"]
    SubMeasurement = SubMeasModels.DNS
    ...
    def prepare_results_no_cache(self, qs: QuerySet):
        ...
        for item in qs.iterator(chunk_size=100):
            ...
            json_data.append(
                {
                    "measurement__raw_measurement__measurement_start_time": datetime.strftime(
                        utc_aware_date(item["time"], self.request.session["system_tz"]),
                        "%Y-%m-%d %H:%M:%S",
                    ),
                    ...
                    "site": (
                        item["measurement__domain__site_id"]
                        if item.get("measurement__domain")
                        and item.get("measurement__domain__site")
                        else -1
                    ),
                    "site_name": (
                        item["measurement__domain__site__name"]
                        if item.get("measurement__domain")
                        and item.get("measurement__domain__site_id")
                        else "(no site)"
                    ),
                    "measurement__anomaly": item["anomaly"],
                    "jsons__answers": self._get_answers(item["jsons__answers"]),
                    "jsons__control_resolver_answers": self._get_control_resolver_answers(
                        item["jsons__control_resolver_answers"]
                    ),
    ...
    def _get_answers(self, answers: List[dict]) -> dict:
        ...
        try:
            ...
            out["A"] = [
                ...
                if answer.get("answer_type") == "A"
            ]
            out["AAAA"] = [
                answer.get("ipv6")
                for answer in answers
                if answer.get("answer_type") == "AAAA"
            ]
            out["cname"] = [
                answer.get("cname")
                for answer in answers
...
class ListHTTPBackEnd(ListSubMeasurementBackend):
    ...
    order_columns = ListSubMeasurementBackend.columns + [
        "status_code_match",
        "headers_match",
        "body_length_match",
        "body_proportion",
...
class ListTORBackEnd(ListSubMeasurementBackend):
    ...
    def filter_queryset(self, qs):
        ...
        dir_port_accessible = get.get("dir_port_accessible")
        if dir_port_accessible:
            dir_port_accessible = int(dir_port_accessible) / 100

        obfs4_accessible = get.get("obfs4_accessible")
        if obfs4_accessible:
            obfs4_accessible = int(obfs4_accessible) / 100

        or_port_dirauth_accessible = get.get("or_port_dirauth_accessible")
        if or_port_dirauth_accessible:
            or_port_dirauth_accessible = int(or_port_dirauth_accessible) / 100

        if dir_port_accessible:
            qs = qs.filter(
                dir_port_accessible__lte=F("dir_port_total") * dir_port_accessible
            )
        if obfs4_accessible:
        ...
        if or_port_dirauth_accessible:
            ...
            print(qs)
            qs = qs.filter(
                or_port_dirauth_accessible__lte=F("or_port_dirauth_total")
                * or_port_dirauth_accessible
            )
            print(qs)
