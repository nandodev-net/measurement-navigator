...
# Local imports
from apps.main.measurements.submeasurements.tasks import \
    count_flags_submeasurements
from apps.main.users import decorators
from vsf.celery import USER_TASK_PRIORITY, transient_queue_name
...
class ControlPanel(VSFLoginRequiredMixin, TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        # try not to run it
        state = cache.get(
            process_names.get(control, self.CONTROL_TYPES.UNKNOWN), ProcessState.UNKNOWN
        )
        if (
            state != ProcessState.IDLE
            and state != ProcessState.FAILED
            and state != ProcessState.UNKNOWN
        ):
        ...
        if control == ControlPanel.CONTROL_TYPES.FASTPATH:
            ...
            fp_update.apply_async(
                kwargs={"until": until, "since": since, "only_fastpath": only_fastpath},
        ...
        # Measurement recovery
            ...
            measurement_update.apply_async(
                queue=transient_queue_name, priority=USER_TASK_PRIORITY
            )
            return JsonResponse({"result": OK})
        # Count flags
