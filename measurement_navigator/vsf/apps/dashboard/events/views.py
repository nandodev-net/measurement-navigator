...
from apps.main.sites.models import Domain, Site
# Inheritance imports
...
class EventsList(VSFLoginRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        current_start_date = get.get("current_start_date") or (
...
class EventsData(VSFLoginRequiredMixin, BaseDatatableView):
    ...
    def filter_queryset(self, qs):
        ...
        current_start_date, current_end_date = self.request.GET.get(
            "current_start_date"
        ), self.request.GET.get("current_end_date")
        ...
        if current_start_date != None and current_start_date != "":
            ...
            utc_start_time = utc_aware_date(
                current_start_date, self.request.session["system_tz"]
            )
            qs = qs.filter(current_start_date__gte=utc_start_time) | qs.filter(
                manual_start_date__gte=utc_start_time
            )

        if current_end_date != None and current_end_date != "":
            current_end_date = datetime.strptime(current_end_date, "%Y-%m-%d")
            utc_end_time = utc_aware_date(
                current_end_date, self.request.session["system_tz"]
            )
            qs = qs.filter(current_end_date__lte=utc_end_time) | qs.filter(
                manual_end_date__lte=utc_end_time
            )
        ...
        if case != None:
            case_events = [
                event.id for event in Case.objects.filter(id=case).first().events.all()
            ]
            ...
            response.append(
                {
                    ...
                    "site": (
                        event.domain.site.name
                        if event.domain and event.domain.site
                        else "No site associated"
                    ),
                    "asn": event.asn.asn,
                    "muted": event.muted,
    ...
    def get(self, request, **kwargs):
        ...
        if eventId != None:
            ...
            data = {
                ...
                "current_start_date": datetime.strftime(
                    utc_aware_date(
                        eventObj.current_start_date, self.request.session["system_tz"]
                    ),
                    "%Y-%m-%d %H:%M:%S",
                ),
                "current_end_date": datetime.strftime(
                    utc_aware_date(
                        eventObj.current_end_date, self.request.session["system_tz"]
                    ),
                    "%Y-%m-%d %H:%M:%S",
                ),
...
class EventDetailView(VSFLoginRequiredMixin, DetailView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            if (
                event.current_start_date != current_start_date
                and event.manual_start_date == None
            ) or (
                event.manual_start_date != current_start_date
                and event.manual_start_date
            ):
            ...
            if (
                event.current_end_date != current_end_date
                and event.manual_end_date == None
            ) or (event.manual_end_date != current_end_date and event.manual_end_date):
                Event.objects.filter(id=post["id"][0]).update(
...
class EditMeasurements(VSFLoginRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        for submeasurement_type in [DNS, TCP, HTTP, TOR]:
            submeasurements_by_type = submeasurement_type.objects.filter(
                event=event
            ).all()
        ...
        context["measurements"] = [
            {
                ...
                "site": (
                    measurement.domain.site.name if measurement.domain.site else None
                ),
