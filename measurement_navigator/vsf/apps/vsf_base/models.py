...
class VsfModel(VsfBaseModel):
    ...
    vsf_code = models.CharField(
        max_length=50, verbose_name="Codigo VSF", unique=True, blank=False, null=False
    )
    ...
    def clean(self):
        ...
        if self.vsf_code:
            if (
                not self.vsf_code
                or not re.match(
                    "^%s\d{9}([A-N]|[P-Z])$" % self.VSF_CODE_PREFIX + date,
                    str(self.vsf_code),
                )
                and not re.match(
                    "^%s\d{5}([A-N]|[P-Z])$" % self.VSF_CODE_PREFIX + date,
                    str(self.vsf_code),
                )
            ):
                raise ValidationError(
                    {
                        "vsf_code": _(
                            "Enter a value with this format %s<5 or 9 digits><1 letter>"
                            % self.VSF_CODE_PREFIX
                            + date
                        )
                    },
                    code="invalid",
