...
def search_measurement_by_queryset(
    ...
    if since != None and since != "":
        measurements = measurements.filter(
            raw_measurement__measurement_start_time__gte=since
        )
...
def anomaly_web_conn(measurement: RawMeasurement) -> bool:
    ...
    return (
        blocking in block_states
        or measurement.test_keys["dns_consistency"] == "inconsistent"
        or not measurement.test_keys["title_match"]
        or not measurement.test_keys["body_length_match"]
        or not measurement.test_keys["headers_match"]
        or not measurement.test_keys["status_code_match"]
    )
...
def anomaly_http_requests(measurement: RawMeasurement) -> bool:
    ...
    """
    return (
        measurement.test_keys.get("body_length_match") != True
    )  # it may be true, false or none
...
def anomaly_tor(measurement: RawMeasurement) -> bool:
    ...
    return (
        dir_port_total != dir_port_accessible
        or obfs4_total != obfs4_accessible
        or or_port_dirauth_total != or_port_dirauth_accessible
        or or_port_total != or_port_accessible
    )
...
def _filter_by_flag_no_ok(qs: QuerySet, subm_to_filter: List[str]) -> QuerySet:
    ...
    if not subm_to_filter:
        return qs

    def filter_aux(subm_type, qs: QuerySet) -> QuerySet:
        field_name = f"{subm_type.__name__}_flag"
        sq = subm_type.objects.filter(measurement=OuterRef("pk")).exclude(
            flag_type=SubMeas.SubMeasurement.FlagType.OK
        )
        return qs.annotate(
            **{field_name: Subquery(sq[:1].values("flag_type"))}
    ...
    qsFinal = None
    if qs1 and not qsFinal:
        qsFinal = qs1
    if qs2 and not qsFinal:
        qsFinal = qs2
    if qs2 and qsFinal:
        qsFinal = qsFinal | qs2
    if qs3 and not qsFinal:
        qsFinal = qs3
    if qs3 and qsFinal:
        qsFinal = qsFinal | qs3
