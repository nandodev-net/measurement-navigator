...
# Local imports
from .models import DNS, HTTP, TCP, TOR, DNSJsonFields, SubMeasurement

# -- SubMeasurement Creation -------------------------------------------------------+


def create_sub_measurements(
    measurement: RawMeasurement,
) -> Tuple[List[SubMeasurement], Dict]:
    """
...
def create_dns_from_webconn(web_con_measurement: RawMeasurement) -> List[DNS]:
    ...
    if web_con_measurement.test_name != RawMeasurement.TestTypes.WEB_CONNECTIVITY:
        raise AttributeError(
            "The given measurement is not a web_connectivity measurement"
        )
    ...
    try:
        cr["answers"] = {"addrs": control_resolver["dns"]["addrs"]}
    except Exception:
        cr["answers"] = None
    ...
    if queries:
        for query in queries:
            ...
            if dns_consistency == "consistent":
                ...
                inconsistent = True
            jsonf = DNSJsonFields.objects.create(
                answers=query["answers"], control_resolver_answers=cr["answers"]
            )
            dns = DNS(
                measurement=None,
                flag=None,
...
def create_dns_from_dns_cons(measurement: RawMeasurement) -> List[DNS]:
    ...
    for query in queries:
        ...
        if (
            query["resolver_hostname"] == cr_ip
            and measurement.input == query["hostname"]
        ):
    ...
    for query in queries:
        if query["resolver_hostname"] != cr_ip and query["query_type"] == "A":
            ...
            if query["resolver_hostname"] in inconsistent:
                ...
                try:
                    if errors[query["resolver_hostname"]] == "no_answer" and not (
                    ...
                    elif errors[query["resolver_hostname"]] == "reverse_match" and not (
                        (control_resolver in errors)
                        or cr["failure"]
                        or control_resolver in failures
                    ):
                        ...
                        try:
                            dns_consistency = (
                                errors[query["resolver_hostname"]] + "(Resolver)"
                            )
                        except:
                            pass
                except KeyError:
...
def create_http_from_web_conn(measurement: RawMeasurement) -> HTTP:
    ...
    # and so other fields
    if (
        (status_code_match is not None)
        and (body_length_match is not None)
        and (headers_match is not None)
        and (body_proportion is not None)
    ):
...
def create_tor_from_tor(measurement: RawMeasurement) -> TOR:
    ...
    if (
        (dir_port_total is not None)
        and (dir_port_accessible is not None)
        and (obfs4_total is not None)
        and (obfs4_accessible is not None)
        and (or_port_dirauth_total is not None)
        and (or_port_dirauth_accessible is not None)
    ):
...
def soft_flag(
    ...
    for MS in meas_types:
        ...
        for i, m in enumerate(measurements.iterator(chunk_size=page_size)):
            ...
            print(
                c.blue(
                    f"Setting flag to {m.flag_type} in measurement of type {MS.__name__}, {i} / {measurement_count}"
                )
            )
        bulker.save()
...
def check_tor_from_tor(tor: TOR) -> bool:
    ...
    if (int(dir_port_accessible) <= int(dir_port_total) // 2) or (
        int(obfs4_total) <= int(obfs4_accessible) // 2
    ):
        return True
    else:
        return False
...
def check_dns_from_web_conn(dns: DNS) -> bool:
    ...
    web_connectivity.
    """
    if dns.ooni_reason_for_blocking:
        return True

    return dns.failure != None and dns.control_resolver_failure == None
