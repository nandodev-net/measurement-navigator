...
# Local imports
from .models import DNS, HTTP, TCP, TOR, SubMeasurement
...
def count_flags_sql():
    ...
    print(
        c.green(
            f"[SUCCESS] Finished flag counting process in {round(end_time - start_time, 4)}s"
        )
    )
...
def _bin_search_max(
    indexable, upper_bound, start: int = 0, end: int = None, key=lambda x: x
) -> int:
...
def _event_creator(
    min_date: datetime, max_date: datetime, asn: ASN, domain: Domain, type: str
) -> Event:
    """
...
def select(
    measurements: List[SubMeasurement],
    ...
    assert (
        event_openning_treshold > 0
    ), "Minimum ammount of measurements to open an event should be a possitive number"
    assert (
        event_continue_treshold <= event_openning_treshold
    ), "Continuation treshold should be less than or equal openning treshold"
    assert (
        event_continue_treshold > 0
    ), "Continuation treshold should be a possitive number"
    ...
    anomaly_count = (
        lambda lo, hi: measurements[hi].previous_counter
    ...
    while n_meas - lo > event_openning_treshold:
        ...
        max_in_date = _bin_search_max(
            measurements, start_time(measurements[lo]) + timedelta, lo, hi, start_time
        )
        ...
        while n_anomalies > event_continue_treshold:
            print(
                c.green(
                    f"\t Taking from {measurements[lo].start_time} to {measurements[max_in_date].start_time}"
                )
            )
            last_index = lo
...
def merge(measurements_with_flags: List[SubMeasurement]):
    ...
    if not measurements_with_flags:
        print(
            c.green(
                "[SUCCESS] no measurements to process, so merge process ended before started"
            )
        )
        return
    ...
    for measurement in measurements_with_flags:
        if measurement.flag_type == soft:
            ...
            measurement.flag_type == hard
            and measurement.event
            and not measurement.event.confirmed
        ):
            hard_flags.append(measurement)
            if resulting_event is None:
                resulting_event = measurement.event
    ...
    if SM_type is None:
        raise TypeError(f"ERROR, THIS IS NOT A SUBMEASUREMENT {reference_measurement}")
...
def hard_flag(
    ...
    """
    submeasurements: List[Tuple[Type[Model], str]] = [
        (DNS, "dns"),
        (HTTP, "http"),
        (TCP, "tcp"),
        (TOR, "tor"),
    ...
    for SM, label in submeasurements:
        ...
        groups = filter(
            ...
            grouper(
                meas.iterator(), lambda m: (m.domain_id, m.probe_asn)
            ),  # group by domain and asn
        )
        ...
        for group in groups:
            ...
            weird_measurements = select(
                group,
                time_window,
                event_openning_treshold,
                interval_size,
                event_continue_treshold,
            )
