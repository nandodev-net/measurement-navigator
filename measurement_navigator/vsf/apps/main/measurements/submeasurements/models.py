...
class SubMeasurement(TimeStampedModel):
    ...
    measurement = models.ForeignKey(
        to=Measurement, on_delete=models.CASCADE, db_index=True
    )
    ...
    time = TimescaleDateTimeField(
        null=True, blank=True, db_index=True, interval="1 week"
    )
    ...
    def save(
        ...
        if self.pk is None:
            ...
            self.flag_type = (
                SubMeasurement.FlagType.SOFT
                if should_flag
                else SubMeasurement.FlagType.OK
            )
    ...
    class Meta:
        abstract = (
            True  # When abstract is True, django wont make a table for this model
        )
...
class DNS(SubMeasurement):
    ...
    )  # añadido por Luis, Andrés debe revisar esto
...
SUBMEASUREMENTS = [HTTP, TCP, TOR, DNS]
