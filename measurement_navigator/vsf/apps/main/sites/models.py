...
class SiteCategory(TimeStampedModel):
    ...
    code = models.CharField(max_length=20, null=False, choices=CodeType.choices)

    old_code = models.CharField(max_length=20, null=False, choices=OldCodeType.choices)

    category_spa = models.TextField()

    category_eng = models.TextField(null=True, blank=True)
...
class Site(TimeStampedModel):
    ...
    name = models.CharField(unique=True, null=False, max_length=40)  # Name of the site
    description_spa = models.TextField()  # A simple spanish description for the site

    description_eng = (
        models.TextField(  # A simple English description for this site (optional)
            null=True, blank=True
        )
    )

    category = models.ForeignKey(
        SiteCategory, null=True, blank=True, on_delete=models.SET_NULL
    )
...
class URL(TimeStampedModel):
    ...
    url = models.URLField(unique=True, null=False, db_index=True)

    # if a site is deleted, set this field to null since a lot of reports would be lost
    site = models.ForeignKey(to=Site, null=True, on_delete=models.SET_NULL)
...
class Domain(TimeStampedModel):
    ...
    site = models.ForeignKey(
        to=Site, null=True, on_delete=models.SET_NULL, related_name="rel_dom"
    )
