...
class InputListConsistensyTest(TestCase):
    ...
    def test_simple_add_ok(self):
        ...
        assert (
            len(Input.objects.filter(asn=asn, input=url)) == 1
        ), "recently added input entry should be available"
    ...
    def test_idempotent_add(self):
        ...
        assert (
            len(Input.objects.filter(asn=asn, input=input)) == 1
    ...
    def test_simple_multi_add_ok(self):
        ...
        assert (
            len(Input.objects.filter(asn=asn1, input=url1)) == 1
        ), "recently added input entry should be available"
        assert (
            len(Input.objects.filter(asn=asn2, input=url2)) == 1
        ), "recently added input entry should be available"
        assert (
            len(Input.objects.filter(asn=asn3, input=url3)) == 1
        ), "recently added input entry should be available"
