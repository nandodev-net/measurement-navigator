...
from .models import EarlyAlertConfig, Emails, Input
...
def update_last_anomaly_rate(
    delta: timedelta = timedelta(days=1), alarming_rate: float = 0.1
):
    """
    Summary:
...
def alert(input: Input, delta: float):
    ...
    print(
        c.red(
            f"[ALERT] {input.input} for asn {input.asn} has an alarming changing rate of {delta}"
        )
    )
