...
class DataProcessor:
    ...
    def get(self, inputs: List[Tuple[url, asn]], only_rate: bool = False):
        ...
        for url_inpt, asn_inpt in inputs:
            if not self.get_measurement(url_inpt, asn_inpt, only_rate):
                print(
                    c.yellow(
                        f"WARNING: could not retrieve data for url {url_inpt}, asn {asn_inpt}"
                    ),
                    file=sys.stderr,
                )
    ...
    def get_measurement(
        self, url_inpt: url, asn_inpt: asn, only_rate: bool = False
    ) -> bool:
        """
        ...
        """
        assert self.start_time and isinstance(
            self.start_time, datetime
        ), "inconsistent DataProcessor object"
        assert self.end_time and isinstance(
        ...
        get_start_time = lambda m: datetime.strptime(
            m["measurement_start_time"][: len(m["measurement_start_time"]) - 1],
            _DATE_FORMAT,
        )
        ...
        while next_url:
            ...
            if req.status_code != 200:
                print(
                    c.yellow(
                        f"WARNING: Could not retrieve measurements for url {url_inpt} with asn {asn_inpt}. Status code: {req.status_code}"
                    ),
                    file=sys.stderr,
                )
                next_url = None
