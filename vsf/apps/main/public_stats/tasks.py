...
@shared_task(time_limit = 10800, vsf_name = 'update_speed_stats', base = VSFTask)   # Task triggered every 12 hrs
def update_speed_stats():
    ...
    first_day_of_the_year_speed_stat = SpeedInternet.objects.filter( day = january_first_object ).first()
    

    data = requests.get('https://statistics.measurementlab.net/v0/SA/VE/' + current_year + '/histogram_daily_stats.json')
    data = data.json()
    ...
    if first_day_of_the_year_speed_stat:
        ...
        speed_internet = SpeedInternet.objects.filter(day = datetime.strptime(element['date'], '%Y-%m-%d')).first()
        if speed_internet:
        ...
        for asn in asns:
            try:
                ...
                print('https://statistics.measurementlab.net/v0/SA/VE/asn/' + asn.asn[2:] + '/' + current_year + '/histogram_daily_stats.json')
                data_asn = requests.get(
                    'https://statistics.measurementlab.net/v0/SA/VE/asn/' + asn.asn[2:] + '/' + current_year + '/histogram_daily_stats.json'
                )
                ...
                if speed_internet_by_asn:
                    speed_internet_by_asn.download_average = float(element['download_AVG'])
                    speed_internet_by_asn.upload_average = float(element['upload_AVG'])
                    speed_internet_by_asn.save()
                    print('----- Last Speed by ASN data from asn ' + asn.asn + ' was updated successfully.')
                else:
                    ...
                    print('----- Last Speed by ASN data from asn ' + asn.asn + ' was created successfully.')
            except:
        ...
        for element in data:
            ...
            same_day = SpeedInternet.objects.filter(day =  datetime.strptime(element['date'], '%Y-%m-%d')).first()
            if same_day:
        ...
        for asn in asns:
            try:
                ...
                print('----- Speed by ASN data from asn ' + asn.asn + ' was charge successfully.')
            except:
                print('----- No ASN ' + asn.asn + ' founded in MLab Database.')
